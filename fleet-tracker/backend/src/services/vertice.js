...
async function loginVertice() {
  try {
    ...
    const loggedIn = Object.keys(cookieJar).some(k => k.includes("Session"));
...
async function buscarSMs() {
  try {
    console.log("[VERTICE] buscarSMs - tem session:", getCookieString().includes("Session"));
    ...
    const params = new URLSearchParams({
      ...
      mDataProp_0: "CD_REGISTRO_VIAGENS", sSearch_0: "", bRegex_0: "false", bSearchable_0: "true",
      mDataProp_1: "CD_REGISTRO_VIAGENS", sSearch_1: "", bRegex_1: "false", bSearchable_1: "true",
      mDataProp_2: "DS_PLACA", sSearch_2: "", bRegex_2: "false", bSearchable_2: "true",
      mDataProp_3: "DS_NOME", sSearch_3: "", bRegex_3: "false", bSearchable_3: "true",
      mDataProp_4: "VL_TOTAL_PRODUTOS", sSearch_4: "", bRegex_4: "false", bSearchable_4: "true",
      mDataProp_5: "DS_PGR", sSearch_5: "", bRegex_5: "false", bSearchable_5: "true",
      mDataProp_6: "DT_INICIO_REAL", sSearch_6: "", bRegex_6: "false", bSearchable_6: "true",
      mDataProp_7: "DT_FIM_REAL", sSearch_7: "", bRegex_7: "false", bSearchable_7: "true",
      mDataProp_8: "VL_SITUACAO", sSearch_8: "", bRegex_8: "false", bSearchable_8: "true",
      mDataProp_9: "NM_RAZAO_SOCIAL", sSearch_9: "", bRegex_9: "false", bSearchable_9: "true",
      mDataProp_10: "DS_CIDADE_ORIGEM", sSearch_10: "", bRegex_10: "false", bSearchable_10: "true",
      mDataProp_11: "DS_CIDADE_DESTINO", sSearch_11: "", bRegex_11: "false", bSearchable_11: "true",
    ...
    console.log("[VERTICE] Grid status:", res.status);
    console.log("[VERTICE] Grid data tipo:", typeof res.data);
    console.log("[VERTICE] Grid data:", JSON.stringify(res.data).substring(0, 500));

    if (!res.data || !res.data.aaData) {
      console.log("[VERTICE] Sem aaData - resetando sessao");
      cookieJar = {};
      return [];
    }
...
async function importarSMsVertice() {
  ...
  for (const sm of sms) {
    try {
      ...
      const { rows: exist } = await db.query("SELECT id FROM viagens WHERE placa = $1 AND status = 'ativa'", [placa]);
